...
async function _ativarReducao(activity) {
  ...
  await actor.setFlag("hunter-system", FLAG, { valor, persistente: escolha.persistente });
...
Hooks.on("combatTurnChange", async (combat, prior, current) => {
  ...
  const red = actor.getFlag("hunter-system", FLAG);
  if ( red?.persistente ) {
    await actor.unsetFlag("hunter-system", FLAG);
    ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ actor }),
...
Hooks.on("deleteCombat", async (combat) => {
  for ( const combatant of (combat?.combatants ?? []) ) {
    ...
    if ( actor.getFlag("hunter-system", FLAG)?.persistente ) {
      await actor.unsetFlag("hunter-system", FLAG);
    }
  }
});
