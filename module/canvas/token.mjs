...
export default class Token5e extends foundry.canvas.placeables.Token {
  ...
  findMovementPath(waypoints, options) {
    ...
    if ( (game.settings.get("hunter-system", "movementAutomation") !== "full") || !this.document.actor?.system.isCreature
  ...
  _getMovementCostFunction(options) {
    ...
    if ( game.settings.get("hunter-system", "movementAutomation") === "none" ) return costFunction;
  ...
  constrainMovementPath(waypoints, options) {
    ...
    ignoreTokens ||= game.settings.get("hunter-system", "movementAutomation") !== "full";
